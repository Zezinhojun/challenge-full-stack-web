...
describe('Student model', () => {
  ...
  beforeEach(() => {
    ...
    jest
      ...
      .mockReturnValue(mockRepository as unknown as Repository<Student>);
    student = new Student();
  });
  ...
  describe('addStudent', () => {
    ...
    it('should throw an error when failing to add a student', async () => {
      ...
      await expect(student.addStudent(mockStudentData)).rejects.toThrow(
  ...
  describe('getAllStudents', () => {
    ...
    it('should throw an error when failing to fetch students.', async () => {
      ...
      await expect(student.getAllStudents()).rejects.toThrow(
  ...
  describe('removeStudent', () => {
    ...
    it('should throw an error when failing to remove a student.', async () => {
      ...
      await expect(student.removeStudent(mockId)).rejects.toThrow(
        `Could not remove student with id ${mockId}`,
      );
    });
  });
  ...
  describe('studentExistsByEmail', () => {
    it('should return a student if exists by email.', async () => {
      ...
      expect(mockRepository.findOne).toHaveBeenCalledWith({
        where: { email: mockStudentData.email },
      });
    ...
    it('should throw an error when failing to check for a student by email.', async () => {
      ...
      await expect(
        student.studentExistsByEmail(mockStudentData.email),
      ).rejects.toThrow('Could not check for student by email');
    });
  });
  ...
  describe('studentExistsById', () => {
    ...
    it('should throw an error when failing to check for a student by id.', async () => {
      ...
      await expect(student.studentExistsById(mockId)).rejects.toThrow(
        'Could not check for student by id',
      );
    });
  });
  ...
  describe('studentExistsByRA', () => {
    ...
    it('should throw an error when failing to check for a student by RA.', async () => {
      ...
      await expect(
        student.studentExistsByRA(mockStudentData.ra),
      ).rejects.toThrow('Could not check for student by ra');
    });
  });
  ...
  describe('studentExistsByCpf', () => {
    ...
    it('should throw an error when failing to check for a student by CPF.', async () => {
      ...
      await expect(
        student.studentExistsByCpf(mockStudentData.cpf),
      ).rejects.toThrow('Could not check for student by CPF');
    });
  });
});
